    ...
    if (this.state.hasError) {
      return (
        <div className="text-center text-red-500" aria-live="assertive">
          Có lỗi xảy ra khi hiển thị sản phẩm mới. Vui lòng thử lại sau.
        </div>
      );
    }
  ...
  const handleSelectAttribute = (productId: string, attributeId: string, valueId: string) => {
    setSelectedAttributes((prev) => {
      ...
      const matchedVariant = product?.variants?.find((variant:any) =>
        variant.attributes.every((attr:any) => updatedAttributes[attr.attribute_id] === attr.value_id)
      );
  ...
  return (
    <ErrorBoundary>
      <section className="py-10 bg-white">
        <h2 className="text-center text-rose-500 font-bold text-3xl mb-6">SẢN PHẨM MỚI</h2>
        <div className="flex flex-wrap justify-center gap-6 px-4">
          {newProducts.length === 0 ? (
            <p className="text-center text-gray-500" aria-live="polite">
              Không tìm thấy sản phẩm mới.
            </p>
          ) : (
