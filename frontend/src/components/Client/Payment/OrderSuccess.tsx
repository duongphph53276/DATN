...
import { CheckCircle, Heart, Star, Gift, Home, Package, ShoppingBag } from 'lucide-react';
...
export default function OrderSuccess({
  orderId: propOrderId = 'N/A',
  totalAmount: propTotalAmount = 0,
  paymentMethod: propPaymentMethod = 'Thanh toán khi nhận hàng',
  ...
  useEffect(() => {
    ...
    const urlTotalAmount = params.get('totalAmount');
    const urlPaymentMethod = params.get('paymentMethod');

    if (urlOrderId) setOrderId(urlOrderId);
    if (urlTotalAmount) setTotalAmount(Number(urlTotalAmount));
    if (urlPaymentMethod) setPaymentMethod(decodeURIComponent(urlPaymentMethod));
          ...
          <div className="bg-white rounded-3xl shadow-xl p-8 mb-6 border-4 border-pink-100">
            <div className="text-center mb-8">
              ...
              <h2 className="text-2xl font-bold text-gray-800 mb-2">
                Gấu bông đang được chuẩn bị cho bạn!
              </h2>
              ...
              <div className="bg-pink-50 rounded-2xl p-6 mb-6">
                ...
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className="font-bold text-purple-600">{paymentMethod}</span>
                </div>
                <div className="flex items-center justify-between mb-4">
                  <span className="font-semibold text-gray-700">Tổng tiền:</span>
                  <span className="font-bold text-green-600 text-xl">{formatVND(totalAmount)}</span>
                </div>
                <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-bold text-orange-600">Đang xử lý</span>
                </div>
              </div>
            </div>
