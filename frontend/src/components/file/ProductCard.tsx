...
const ProductCard = ({ id, img, name, price, oldPrice, size }: ProductCardProps) => {
  const [selectedSize, setSelectedSize] = useState<string | null>(null);

  return (
    <div className="w-[250px] bg-white p-4 rounded-xl shadow hover:shadow-lg text-center flex flex-col">
      <img src={img} alt={name} className="w-full h-[220px] object-cover rounded-lg mb-4" />
      ...
      <Link
        to={`/product/${id}`}
        className="inline-block mt-2 px-4 py-1 bg-rose-500 text-white rounded hover:bg-rose-600 transition"
      >
        Xem chi tiết
      </Link>
    </div>
